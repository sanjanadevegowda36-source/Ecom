...
import { FaTasks, FaCheckCircle, FaClock, FaExclamationCircle, FaUser, FaEnvelope, FaPhone, FaEdit, FaTrash, FaBriefcase } from 'react-icons/fa';
import { useProductContext } from '../context/ProductContext';
import './ProfessionalDashboard.css';

/**
 * ProfessionalDashboard - Dashboard for professionals to view and manage assigned tasks
 * Professionals can only view tasks assigned to them, not the full admin dashboard
 */
const ProfessionalDashboard = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      const professionalTasks = getWorkerTasks(user.email);
      setTasks(professionalTasks);
    }
  }, [user, getWorkerTasks]);
  ...
  // Get orders that can be managed by this professional
    ...
    <div className="professional-dashboard-container">
      <div className="professional-dashboard-header">
        <div className="header-info">
          <h1><FaBriefcase /> Professional Dashboard</h1>
          <p>Welcome back, {user?.name || user?.email.split('@')[0]}!</p>
        </div>
        <div className="professional-info-card">
          <FaUser className="professional-avatar" />
          <div className="professional-details">
            <span className="professional-name">{user?.name}</span>
            <span className="professional-email">{user?.email}</span>
            <span className="professional-role">Professional</span>
          </div>
        </div>
      </div>
